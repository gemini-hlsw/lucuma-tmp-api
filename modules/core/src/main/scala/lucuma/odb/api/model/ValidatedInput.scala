...
object ValidatedInput {
  ...
  def optionEither[A, B](
    nameA: String,
    nameB: String,
    a:     Option[ValidatedInput[A]],
    b:     Option[ValidatedInput[B]]
    ...
    List(a.map(_.map(_.asLeft[B])), b.map(_.map(_.asRight[A]))).flattenOption match {
      ...
      case _       =>
        InputError.fromMessage(s"Either $nameA or $nameB are permitted but not both").invalidNec
    }
  ...
  def closedInterval[T](name: String, value: T, low: T, high: T)(implicit
